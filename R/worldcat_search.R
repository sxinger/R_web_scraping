get_worldcat_full<-function(query,max_return=2000,count=10,api_key,verb=T){
 #TODO
}


worldcat_scrape<-function(keyword1,keyword2){
  #--build query grid
  ...
  liter_worldcat<-list(liter_data=liter_data,
                  liter_meta=liter_meta)
  
  return(liter_worldcat)
}