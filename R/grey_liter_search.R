get_grey_liter_full<-function(query,max_return=2000,count=10,api_key,verb=T){
 #TODO
}


grey_liter_scrape<-function(keyword1,keyword2){
  #--build query grid
  ...
  liter_grey_liter<-list(liter_data=liter_data,
                         liter_meta=liter_meta)
  
  return(liter_grey_liter)
}